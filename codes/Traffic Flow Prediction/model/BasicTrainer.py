...
class Trainer(object):
    ...
    def train(self):
        ...
        #save train_loss figure
        if not self.args.debug:
            epo = np.arange(1, len(train_loss_list) + 1)
            plt.plot(epo, train_loss_list)
            plt.savefig(self.loss_figure_path)
            self.logger.info("Saving the train loss figure to " + self.loss_figure_path)
